  ...
  (when (display-graphic-p)
    (let* ((cache (expand-file-name "theme-cache.el" user-emacs-directory))
      ...
      (with-temp-file cache
        ...
        (when fs (insert (format "(push '(fullscreen . %S) default-frame-alist)\n" fs)))
        (when w  (insert (format "(push '(width . %S) default-frame-alist)\n" w)))
        (when h  (insert (format "(push '(height . %S) default-frame-alist)\n" h)))
        (when (and l (integerp l))
          (insert (format "(push '(left . %S) default-frame-alist)\n" l)))
        (when (and tp (integerp tp))

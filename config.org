...
(defvar-local kwarks--tree-hidden-nodes (make-hash-table :test 'equal)
...
;;   (save-excursion
;;     (let ((has-children (file-directory-p node))
;;           (dots-overlay (make-overlay (line-beginning-position) (line-end-position)))
;;           (collaped-p nil))
;;       (forward-line 1)
;;       (while has-children
...
;;                (line-end (point))
;;                (parent (get-text-property (point-min) 'parent line)))
;;           (setq has-children (string-prefix-p node parent))
;;           (if has-children
;;               (progn 
;;                 (setq collaped-p t)
;;                 (kwarks--tree-hide-node
;;                  (if collapse-children-p parent node) line-start line-end)))))
;;       (if collaped-p
;;           (progn
;;             (puthash node (cons dots-overlay (gethash node kwarks--tree-hidden-nodes)) kwarks--tree-hidden-nodes)
;;             (overlay-put dots-overlay 'after-string "..."))))))
    ...
    (let ((child-node-p t))
      ...
      (while child-node-p
        (let* ((line-start (point))
               ...
               (line-end (point))
               (parent (get-text-property (point-min) 'parent line)))
          (setq child-node-p (string-prefix-p node parent))
          (if child-node-p
              (kwarks--tree-hide-node
               (if collapse-children-p parent node) line-start line-end)))))))

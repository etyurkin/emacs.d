...
;; Capture the frame size desktop just restored.  Theme, font and modeline
;; changes that follow will resize the frame; re-assert the correct size
;; once startup is complete.  Position is left to macOS (it already placed
;; the frame on the right display).
(defvar kwarks/post-desktop-size nil)
(when (display-graphic-p)
  (let ((frame (selected-frame)))
    (setq kwarks/post-desktop-size
...
(add-hook 'emacs-startup-hook
          (lambda ()
            (run-with-idle-timer 0.5 nil
              (lambda ()
                (when (and kwarks/post-desktop-size (display-graphic-p))
                  (modify-frame-parameters (selected-frame)
                                           kwarks/post-desktop-size))))))

...
;; Desktop's frameset restores frame geometry in character units, so the
;; restored frame ends up a few pixels off once the final font/modeline are
;; applied.  Capture the exact PIXEL geometry at save time (font-independent)
;; and re-assert it after startup.  Persisted via `desktop-globals-to-save'.
(defvar kwarks/frame-geometry nil
  "Saved pixel geometry of the main frame, restored on the next startup.")

(defun kwarks/save-frame-geometry (&rest _)
  "Capture the selected frame's pixel size and position."
    ...
    (let ((frame (selected-frame)))
      (setq kwarks/frame-geometry
            (list (cons 'fullscreen   (frame-parameter frame 'fullscreen))
                  ...
                  (cons 'pixel-width  (frame-text-width frame))
                  (cons 'pixel-height (frame-text-height frame)))))))

(defun kwarks/restore-frame-geometry ()
  "Re-apply the saved pixel geometry to the selected frame."
  (when (and kwarks/frame-geometry (display-graphic-p))
    (let* ((g  kwarks/frame-geometry)
           (fs (cdr (assq 'fullscreen g)))
           (frame (selected-frame)))
      (if fs
          (set-frame-parameter frame 'fullscreen fs)
        (modify-frame-parameters
         frame
         (list (cons 'fullscreen nil)
               (cons 'left   (cdr (assq 'left g)))
               (cons 'top    (cdr (assq 'top g)))
               (cons 'width  (cons 'text-pixels (cdr (assq 'pixel-width g))))
               (cons 'height (cons 'text-pixels (cdr (assq 'pixel-height g))))))))))

;; Capture right before the desktop is written (covers exit and auto-save).
(advice-add 'desktop-save :before #'kwarks/save-frame-geometry)
;; Re-assert after startup once the font/theme/modeline have settled.

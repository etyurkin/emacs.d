...
;; Capture the geometry desktop just restored.  Theme, font and modeline
;; changes that follow will resize the frame; re-assert the correct
;; geometry once startup is complete and macOS has settled.
(defvar kwarks/post-desktop-geometry nil)
(when (display-graphic-p)
  (let ((frame (selected-frame)))
    (setq kwarks/post-desktop-geometry
...
(add-hook 'emacs-startup-hook
          (lambda ()
            (run-with-idle-timer 0.5 nil
              (lambda ()
                (when (and kwarks/post-desktop-geometry (display-graphic-p))
                  (modify-frame-parameters (selected-frame)
                                           kwarks/post-desktop-geometry))))))

...
TODO: heck osx version with 'sw_vers -productVersion' and apply fix if version is 10.12.*
...
(defun apply-theme ()
  ...
  ;; selection color
  (set-face-attribute 'region nil :background "#376da4")
...
(use-package wttrin
  ...
  (defun weather ()
    (interactive)
    ;; use default city, enforce metric units (?u for imperial)
    (wttrin-query (concat (car wttrin-default-cities) "?m"))
    (rename-buffer (concat "*Weather - " (car wttrin-default-cities) "*"))

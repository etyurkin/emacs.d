...
(defun init-doom-modeline ()
  ...
  (doom-modeline-def-modeline 'my-simple-line
    ...
    '(buffer-position selection-info minor-modes process vcs checker input-method buffer-encoding fancy-battery misc-info major-mode))
...
(defun kwarks/helm-top-sh (orig-fn &rest args)
  ...
  (let ((default-directory "/sudo::")
    ...
    (message "kill -s %s %s exited with status %s"
             ...
             (apply (lambda (pid)
                      (shell-command-to-string (format "kill -s %s %s" sig pid)))
                    pids))
    (dolist (buffer (buffer-list))
      (when (or (string-match "^*helm top*" (buffer-name buffer))
                (string-match "^*tramp/sudo" (buffer-name buffer)))
        (kill-buffer buffer)))))
...
(defun ivy-kill-buffer ()
  ...
  (interactive)
  (setq this-command #'ivy-kill-buffer)
  ...
  (defmacro helm-know-your-http-well (source selector-fn)
    `(helm :sources (helm-build-sync-source "helm-know-your-http-well"
		              ...
		              :action (lambda (candidate)
				                (message "%s: %s" (caar candidate) (funcall ,selector-fn candidate))))
  ...
  (defun helm-http-status-code ()
    ...
    (helm-know-your-http-well
     (mapcar (lambda (arg) ; combine http status with description
               (let ((title (concat (car arg) " - " (caadr arg))))
                 (list title (cons title (cdadr arg)))))

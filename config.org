...
(defun kwarks--make-tree (root)
  (let ((cmd (format "tree %s --dirsfirst --noreport -J" root))
        (json-object-type 'plist)
        (json-array-type 'list)
        (json-key-type 'symbol))
    (json-read-from-string (shell-command-to-string cmd))))

(defun kwarks--print-tree-node (node is-directory is-link)
  (cond (is-directory (propertize node 'font-lock-face 'org-level-1))
        (is-link (propertize node 'font-lock-face '(org-level-3 italic)))
        (t (propertize node 'font-lock-face 'org-level-3))))
  ...
  (while tree
    (let* ((record (car tree))
      ...
      (if is-root
          (insert name)
        (insert
         (format "%s%s %s" spacing prefix
                 (kwarks--print-tree-node
                  (propertize name
                              'path full-name
                              'help-echo full-name
...
(defun kwarks/tree ()
  (interactive)
  (counsel--find-file-1
   "Root path: " (kwarks--get-buffer-directory)
   (lambda (dir)
     (let* ((buffer-name "*tree*"))
       (get-buffer-create buffer-name)
       (set-buffer buffer-name)
       (read-only-mode -1)
       (erase-buffer)
       (kwarks--print-tree (kwarks--make-tree dir) "" "")
       (goto-char (point-min))
       (special-mode)

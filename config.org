...
;; resizes our frame about a second into the session (it fires a move/focus
;; event and rescales the window), shrinking it below the size desktop
;; restored.  This is an external (window-system) resize, so
;; `frame-inhibit-implied-resize' cannot stop it.  Instead we capture the
;; correct pixel size once startup is complete (before macOS interferes) and
;; re-assert it whenever the frame size changes, for the first few seconds.
(defvar kwarks/target-frame-pixels nil
  "Target (WIDTH . HEIGHT) in pixels, re-asserted while the layout settles.")
(defvar kwarks/stabilizing nil
  "Non-nil while we are programmatically resizing, to avoid recursion.")

(defun kwarks/stabilize-frame-size (&rest _)
  "Snap the frame back to `kwarks/target-frame-pixels' if macOS shrank it."
  (when (and kwarks/target-frame-pixels
             (not kwarks/stabilizing)
             (display-graphic-p)
             (not (frame-parameter (selected-frame) 'fullscreen)))
    (let* ((f  (selected-frame))
           (tw (car kwarks/target-frame-pixels))
           (th (cdr kwarks/target-frame-pixels)))
      (when (or (> (abs (- (frame-pixel-width f) tw)) 2)
                (> (abs (- (frame-pixel-height f) th)) 2))
        (let ((kwarks/stabilizing t))
          (set-frame-size f tw th t))))))

(add-hook 'emacs-startup-hook
          (lambda ()
            (when (display-graphic-p)
              (let ((f (selected-frame)))
                (setq kwarks/target-frame-pixels
                      (cons (frame-pixel-width f) (frame-pixel-height f))))
              (add-hook 'window-size-change-functions #'kwarks/stabilize-frame-size)
              (add-hook 'move-frame-functions #'kwarks/stabilize-frame-size)
              ;; Let go once things have settled so manual resizing sticks.
              (run-with-idle-timer
               5 nil
               (lambda ()
                 (remove-hook 'window-size-change-functions #'kwarks/stabilize-frame-size)
                 (remove-hook 'move-frame-functions #'kwarks/stabilize-frame-size))))))

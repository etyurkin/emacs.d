...
(defun ivy--make-process-list (str collection predicate)
  (mapcar (lambda (ps)
            (let ((ps-info (split-string (string-trim ps))))
              (format "%-6s %-20s %s" (first ps-info) (second ps-info) (third ps-info))))
          (cdr (split-string (shell-command-to-string "ps -eo pid,user,command") "\n+"))))

(defun ivy--kill-process (process)
  (let ((default-directory "/sudo::")
        (pid (car (split-string (string-trim process) "\s+"))))
    (print (format "killing %s" pid))
    (shell-command (format "kill %s %s" -9 pid))))

(defun ivy-kill-process ()
  (interactive)
